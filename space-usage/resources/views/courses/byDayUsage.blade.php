...
@section('content')
@php
    $sectionsData = $dayType === 'tuth' ? $sectionsDataTuTh : $sectionsDataMWF;
@endphp
    ...
    @if($hasAllFilters)
        <div id="results">
            ...
            <ul class="nav nav-tabs mb-3" id="viewTabs" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="table-view-tab" data-bs-toggle="tab" data-bs-target="#table-view" type="button" role="tab" aria-controls="table-view" aria-selected="true">Table View</button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="compare-view-tab" data-bs-toggle="tab" data-bs-target="#compare-view" type="button" role="tab" aria-controls="compare-view" aria-selected="false">Compare View</button>
                </li>
            </ul>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" id="blockPerDay" class="form-control" value="{{ $dayType === 'tuth' ? 6 : 9 }}" min="1" step="0.1">
                    </div>

                    @if ($sectionsData->isEmpty())
                        <p class="text-muted">No sections for {{ $dayType === 'mwf' ? 'MWF' : 'TuTh' }} with the selected filters.</p>
                    @else
                            ...
                            <table class="table table-striped table-hover table-sm sticky-header-table">
                                ...
                                <tbody id="coursesTableBody">
                                    ...
                                    <tr class="course-row"
                                        ...
                                        <td class="forecast-blocks-per-week">{{ $section['daysPerWeek'] * ceil($section['duration_minutes'] / ($dayType === 'tuth' ? 75 : 50)) }}</td>
            ...
            <div class="tab-pane fade" id="compare-view" role="tabpanel" aria-labelledby="compare-view-tab">
                <div class="card">
                    <div class="card-body">
                        <h2 class="mb-4">Seat Range Comparison ({{ $dayType === 'mwf' ? 'MWF' : 'TuTh' }})</h2>
                        <p class="text-muted mb-4">
                            <strong>Calculated Count</strong> is the sum of "Rooms Needed" for all sections in each seat range (blocks needed / total blocks available).
                            <strong>Current Count</strong> is the number of unique rooms per campus in that seat range.
                        </p>
                        @php $rangeLabels = ['0-25', '26-49', '50-74', '75-124', '125-174', '175-224', '225-249', '250-299', '300-349', '350-399', '400+']; @endphp
...
@if($hasAllFilters && !$sectionsData->isEmpty())
<script>
(function() {
    ...
    function updateCompareView() {
        ...
        rangeLabels.forEach(function(r) { calculatedRanges[r] = 0; });
        document.querySelectorAll('.course-row').forEach(function(row) {
            const seatingRange = row.querySelector('.forecast-seating-range');
            const roomsNeededCell = row.querySelector('.forecast-rooms-needed');
            const seatingRangeText = seatingRange ? seatingRange.textContent.trim() : '';
            const roomsNeeded = parseFormattedNumber(roomsNeededCell ? roomsNeededCell.textContent.trim() : 0);
            if (seatingRangeText && seatingRangeText !== 'N/A' && calculatedRanges.hasOwnProperty(seatingRangeText)) {
                calculatedRanges[seatingRangeText] += roomsNeeded;
            }
        });
        ...
        var totalCurrent = 0;
        rangeLabels.forEach(function(range) {
            ...
            const currentCell = row.querySelector('.current-count');
            const current = currentCell ? parseFloat(currentCell.textContent.trim() || 0) : 0;
            const difference = current - calculatedRounded;
            totalCalculated += calculatedRounded;
            totalCurrent += current;
            const calcCell = row.querySelector('.calculated-count');
            if (calcCell) calcCell.textContent = calculatedRounded;
        ...
        if (totalRow) {
            ...
            const totalCalcEl = totalRow.querySelector('.total-calculated');
            const totalDiffEl = totalRow.querySelector('.total-difference');
            if (totalCalcEl) totalCalcEl.textContent = totalCalculated;
            if (totalDiffEl) {
    ...
    document.querySelector('#compare-view-tab')?.addEventListener('shown.bs.tab', function() { updateCompareView(); });
